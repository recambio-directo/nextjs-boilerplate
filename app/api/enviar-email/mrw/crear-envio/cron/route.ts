// app/api/mrw/cron/route.ts
// Cron job — llamado por Vercel cada 30 minutos
// Consulta el estado de todos los pedidos con tracking MRW activo
...
export async function GET(request: Request) {
  ...
  try {
    // Buscar pedidos con tracking MRW activo (estado enviado o preparando)
    ...
    for (const pedido of pedidos) {
      try {
        // Consultar estado en MRW
        const trackingRes = await fetch(`${baseUrl}/api/mrw/tracking`, {
          method: "POST",
                      ...
                      <div style="background:white;padding:32px;border-radius:12px;max-width:600px;margin:auto;">
                        ...
                        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px;margin:20px 0;">
                          <p style="margin:0;color:#166534;font-size:14px;">
                            📦 Tracking MRW: <strong>${pedido.tracking}</strong>
                          </p>
                        </div>
                ...
                html: `
                  <div style="font-family:Arial;padding:30px;background:#f3f4f6;">
                    <div style="background:white;padding:32px;border-radius:12px;max-width:600px;margin:auto;">
                      ...
                      <p style="color:#374151;font-size:15px;">El pedido <strong>${pedido.codigo}</strong> ha sido entregado. Debes pagar al proveedor en 7 días.</p>
    ...
    console.error("Error cron MRW:", error);
